  ...
  return `
    <main id="main" tabindex="-1">
      ...
      <section id="pricing" class="pricing section-rule" aria-labelledby="pricing-heading">
        <div><p class="eyebrow">Studio</p><h2 id="pricing-heading">Check Studio billing status.</h2><p>Studio includes agency rooms, private reviewers, GitHub fixture import, and exports. Client reviewers use a private room link without an account or subscription.</p></div>
...
function privacy(): string {
  return `<main id="main" tabindex="-1" class="document-page"><p class="eyebrow">Privacy</p><h1 id="page-heading" tabindex="-1">Your sample stays separate.</h1><p class="document-lede">The demo is a browser-only sample. Real rooms use your Sociobot account and private server storage.</p><section><h2>What the sample stores</h2><p>The demo saves its room, checklist, and acknowledgement in one local key beginning with <code>demo:</code>. Reset demo replaces it.</p></section><section><h2>What the sample does not send</h2><p>The demo sends no fixture, checklist, or acknowledgement data. It uses no cookies, analytics, third-party fonts, or third-party scripts.</p></section><section><h2>What real rooms store</h2><p>Real rooms store your agency name, selected repository reference, sanitized fixture, questions, and acknowledgement. GitHub OAuth tokens are encrypted before storage and are removed when you disconnect GitHub or delete your agency.</p><p>Agency members can download each room export. Signed-in agency owners can permanently delete the agency, rooms, client links, and connected repositories from <a href="/settings/data">Data controls</a>.</p></section><section><h2>Sign-in and checkout</h2><p>Microsoft Entra handles sign-in. Sociobot handles hosted checkout only after its product registration is live. This product does not receive your password or card details.</p></section><p><a href="/terms">Read the terms</a></p></main>`;
}

function terms(): string {
  return `<main id="main" tabindex="-1" class="document-page"><p class="eyebrow">Terms</p><h1 id="page-heading" tabindex="-1">A review is not a contract.</h1><p class="document-lede">Use the sample for evaluation. Use real rooms only for fixtures you are allowed to share with the invited client.</p><section><h2>Acknowledgement</h2><p>A named acknowledgement says the reviewer completed the displayed checklist for one revision. It is not an e-signature, legal approval, or substitute for a contract.</p></section><section><h2>Studio subscription</h2><p>Studio is $79 USD per agency each month when Sociobot registration is available. Client reviewers use a private room link without an account or subscription. A checkout link appears only after Sociobot has registered and verified the hosted subscription product.</p><p>Studio registration is not available yet. This page cannot start a payment or grant a subscription until the operator completes that setup.</p></section><section><h2>Repository use</h2><p>GitHub App consent is read-only and limited to repositories selected in GitHub. Select a connected repository again in this product before its JSON fixture can be imported. Disconnect removes the encrypted connection token.</p></section><p><a href="/privacy">Read the privacy boundary</a></p></main>`;
}
...
function billingPage(): string {
  ...
  const studioPrice = config?.studio_price ?? "$79 USD per agency each month";
