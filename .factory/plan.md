...
- **Delivery:** a static web deployment serves the Vite build and a Container Apps API serves `/api`; the factory owns DNS and deployment configuration. `staticwebapp.config.json` supplies SPA fallback and security headers. The API CORS allowlist only permits the production web origin and localhost development origin.
